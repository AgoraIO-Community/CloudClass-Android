        ...
        synchronized(lockObject) {
            ...
            launchConfig?.let {
                ...
                AgoraClassroomSDK.launch(context, launch) {
                    //fullLoading.dismiss()
                    Constants.AgoraLog?.i("Group 3、launch:" + it.value)

                    // 保留大房间数据
                    if (it == AgoraEduEvent.AgoraEduEventReady && !isChangeGroup) {
                    ...
                    onAcceptListener?.invoke(groupInfo.groupUuid)
                }
            }
        }
    }
