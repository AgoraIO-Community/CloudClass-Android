...
abstract class AgoraEduClassActivity : AgoraBaseClassActivity(), IAgoraUIProvider, IAgoraConfig {
                    ...
                    if (lastLocalhasVideo != curlocalhasVideo) {//如果上一次的视频状态和本次的视频状态不一样
                        when (streamInfo.streamType) {
                            ...
                            AgoraEduContextMediaStreamType.Audio, AgoraEduContextMediaStreamType.None -> {
                                AgoraUIToast.error(applicationContext, text = String.format(context.getString(R.string.fcr_stream_stop_video)))
                    ...
                    if (lastLocalhasAudio != curlocalhasAudio) {
                        when (streamInfo.streamType) {
                            ...
                            AgoraEduContextMediaStreamType.Video, AgoraEduContextMediaStreamType.None -> {
                                AgoraUIToast.error(applicationContext, text = String.format(context.getString(R.string.fcr_stream_stop_audio)))
    ...
    fun setDualStreamMode(){
        // 设置双流
        val lowStreamWidth = 320
        val lowStreamHeight = 240
        val lowStreamFrameRate = 15
        val lowStreamBitrate = 200

        val lowStreamValue = "{\"width\":$lowStreamWidth,\"height\":$lowStreamHeight,\"frameRate\":$lowStreamFrameRate,\"bitRate\":$lowStreamBitrate}"
        val lowStream = "{\"che.video.lowBitRateStreamParameter\":$lowStreamValue}"
        eduCore()?.eduContextPool()?.streamContext()?.setRtcParameters(lowStream)
        eduCore()?.eduContextPool()?.streamContext()?.enableDualStreamMode(true)
    }
    ...
    val myRoomHandler = object : RoomHandler() {
        ...
        override fun onJoinRoomFailure(roomInfo: EduContextRoomInfo, error: EduContextError) {
            ...
            Constants.AgoraLog?.e("$TAG->classroom ${roomInfo.roomUuid} joined fail:${error.msg} || ${error.code}")
        }
                ...
                FCRGroupClassUtils.mainLaunchConfig?.let { launch ->
                    ...
                    AgoraClassroomSDK.launch(context, launch) {
                        ...
                        eduCore()?.eduContextPool()?.roomContext()?.getRoomInfo()?.roomUuid?.let {
                            Constants.AgoraLog?.e("Media launchMainRoom-> ${it} reset media")
