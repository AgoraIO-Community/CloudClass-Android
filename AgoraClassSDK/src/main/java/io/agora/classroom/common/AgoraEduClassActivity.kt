...
import io.agora.classroom.helper.FcrRtcParameters
...
abstract class AgoraEduClassActivity : AgoraBaseClassActivity(), IAgoraUIProvider, IAgoraConfig {
    ...
    fun setDualStreamMode(){
        val lowStream = GsonUtil.gson.toJson(FcrRtcParameters())
        Constants.AgoraLog?.i("joinRoom lowStream = $lowStream")
        eduCore()?.eduContextPool()?.streamContext()?.setRtcParameters(lowStream)
        eduCore()?.eduContextPool()?.streamContext()?.enableDualStreamMode(true) // 设置双流
    }
