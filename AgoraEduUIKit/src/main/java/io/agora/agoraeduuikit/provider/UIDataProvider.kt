...
class UIDataProvider(private val eduContext: EduContextPool?) {
    ...
    fun setGrantedUsers(eduCore: AgoraEduCore?){
        try {
            val map =  (((eduCore?.room()?.roomProperties?.get("widgets") as? Map<*,*>)?.
            get("netlessBoard") as? Map<*,*>)?.
            get("extra") as? Map<*,*>)?.get("grantedUsers") as? Map<String, Boolean>
...
//                    Constants.AgoraLog?.i("$tag->${packet.signal}, LargeWindowStopRender")
//                }
            }
        }
    }
...
//        }
        ...
        val userDetailInfo = toAgoraUserDetailInfo(streamInfo.owner,
    ...
    fun toAgoraUserDetailInfo(user: AgoraEduContextUserInfo,
                              isCoHost: Boolean,
