...
internal object AgoraUIToast {
    ...
    private fun showToast(context: Context, level: Int, anchor: View?, text: String, duration: Int) {
        ContextCompat.getMainExecutor(context).execute {
            ...
            toastLayout.findViewById<RelativeLayout>(R.id.agora_toast_layout)?.let { layout ->
                ...
                buildToastBgDrawable(level)?.let { drawable ->
    ...
    private fun buildToastBgDrawable(level: Int): Drawable? {
        val bgColor: Int
        val borderColor: Int
        when (level) {
            LEVEL_INFO -> {
