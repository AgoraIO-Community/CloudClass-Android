...
import io.agora.agoraeducore.core.context.EduContextRoomType.OneToOne
import io.agora.agoraeducore.core.context.EduContextRoomType.SmallClass
import io.agora.agoraeducore.core.context.EduContextRoomType.LectureHall
...
class AgoraEduFloatingControlWindow(
    ...
    context: Context,
    private val optionListener: IAgoraOptionListener2,
    ...
    private fun initStyleDialog(anchor: View?) {
        ...
        anchor?.let {
            ...
            if (eduContext?.roomContext()?.getRoomInfo()?.roomType == OneToOne) {
...
private class OptionItemProviderFactory(eduContext: EduContextPool?) : OptionItemProvider(eduContext) {
    ...
    private fun buildProvider(eduContext: EduContextPool?): OptionItemProvider {
        return when (eduContext?.roomContext()?.getRoomInfo()?.roomType) {
            OneToOne -> { OptionItemProvideStrategyByOneToOne(eduContext) }
            SmallClass -> { OptionItemProvideStrategyBySmall(eduContext) }
            LectureHall -> { OptionItemProvideStrategyByLecture(eduContext) }
