    ...
    fun upsertUserDetailInfo(info: EduContextUserDetailInfo) {
        ...
        this.view.post {
            ...
            if (!currentVideoOpen && newVideoOpen) {
                videoListener?.onRendererContainer(videoContainer, info.streamUuid, EduContextRenderConfig())
            } else if (currentVideoOpen && !newVideoOpen) {
                videoListener?.onRendererContainer(null, info.streamUuid, EduContextRenderConfig())
            } else {
                val parent = if (newVideoOpen) videoContainer else null
                videoListener?.onRendererContainer(parent, info.streamUuid, EduContextRenderConfig())
            }
