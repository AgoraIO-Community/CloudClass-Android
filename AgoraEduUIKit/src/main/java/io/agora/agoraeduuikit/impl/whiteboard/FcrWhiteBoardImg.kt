...
class FcrWhiteBoardImg(var context: Context) {
    ...
    fun saveImgToGallery(room: Room) {
        if (isRunning.get()) {
            Constants.AgoraLog?.e("$TAG->已经在保存白板图片")
            return
        }
        index = 0
        isRunning.set(true)
        room.getEntireScenes(object : Promise<MutableMap<String, Array<Scene>>> {
            override fun then(t: MutableMap<String, Array<Scene>>?) {
                ...
                t?.get("/")?.forEach { // 获取主板的截图
                    Constants.AgoraLog?.e("$TAG->白板scenePath：" + "/" + it.name)
                   ...
                   room.getSceneSnapshotImage(scenePath, object : Promise<Bitmap> {
                        override fun then(t: Bitmap?) {
                            ...
                            if (count == list.size) {
                                ...
                                Constants.AgoraLog?.e("$TAG->白板图片分${bitMap.size}张图存储，总共：$count 张图")
                                ...
                                bitMap.forEach { map ->
                                    currentIndex++
                                    Constants.AgoraLog?.e("$TAG->开始合成白板第:${map.key}个图>>>>>>")
                                    ...
                                    if (bitmap == null) {
                                        ...
                                        ImageUtils.saveToGallery(context, bitmap, getImageName()) {
                                            if (currentIndex == size) {
                                                if (it) {
                                                    Constants.AgoraLog?.e("$TAG->白板第:${map.key}个图合成完成")
                                                    AgoraUIToast.info(
                                                        context,
                                                        textResId = R.string.fcr_savecanvas_tips_save_successfully
                                                    )
                                                } else {
                                                    Constants.AgoraLog?.e("$TAG->白板第:${map.key}个图合成完成失败")
                                                    AgoraUIToast.info(
                                                        context,
                        ...
                        override fun catchEx(t: SDKError?) {
                            Constants.AgoraLog?.e("$TAG->获取白板图片失败：${t?.message}")
                            isRunning.set(false)
                            AgoraUIToast.info(context, textResId = R.string.fcr_savecanvas_tips_save_failed)
                        }
                    })
            ...
            override fun catchEx(t: SDKError?) {
                Constants.AgoraLog?.e("$TAG->获取白板路径失败：${t?.message}")
                isRunning.set(false)
                AgoraUIToast.info(context, textResId = R.string.fcr_savecanvas_tips_save_failed)
            }
        })
