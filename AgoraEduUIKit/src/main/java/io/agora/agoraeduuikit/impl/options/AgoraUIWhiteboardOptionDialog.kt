...
class AgoraUIWhiteboardOptionDialog(context: Context,
    ...
    private val calculator = DialogSizeCalculator()
    private var pivot = Pair(0f, 0f)
    private val animateUtils: PopupAnimationUtil = PopupAnimationUtil()
    ...
    fun show(container: ViewGroup, anchor: View, margin: Int) {
        ...
        super.show()
        animateUtils.runShowAnimation(layout, pivot.first, pivot.second)
    }

    override fun dismiss() {
        animateUtils.runDismissAnimation(layout, pivot.first, pivot.second) {
            super.dismiss()
            this.anchor = null
        }
    }
    ...
    private fun initView() {
        ...
        layout = findViewById(R.id.agora_option_whiteboard_dialog_shadow_bg)
    ...
    private fun adjustPosition(anchor: View, width: Int, height: Int, margin: Int) {
        ...
        // Calculate scaling pivot according to the center
        // position of the anchor view.
        val pivotX = params.width.toFloat()
        val pivotY = params.height - anchor.height.toFloat() / 2
        pivot = Pair(pivotX, pivotY)
    }
}
