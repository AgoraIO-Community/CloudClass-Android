...
class AgoraWhiteBoardWidget : AgoraBaseWidget() {
    ...
    private var coursewareAttributes: String? = null //把课件保存下来，带到分组的时候用到
            ...
            RetrofitManager.instance().getService(AgoraEduSDK.baseUrl(), BoardService::class.java)
                ...
                .enqueue(RetrofitManager.Callback(0, object : ThrowableCallback<ResponseBody<Any>> {
                    override fun onSuccess(res: ResponseBody<Any>?) {
                        ...
                        AgoraLog?.e("$tag-> getCoursewareAttribute ${coursewareAttributes}")
                        try {
                            this@AgoraWhiteBoardWidget.boardProxy.setWindowManagerAttributes(coursewareAttributes)
...
//        params.isWritable = widgetInfo?.localUserInfo?.userRole == AgoraEduRoleTypeTeacher.value
        ...
        BoardUtils.registerTalkative(whiteBoardControlView.context, whiteSdk) // joinRoom 之前
                    ...
                    override fun then(t: Boolean?) {
                        ...
                        if (t == true && roomContext?.getRoomInfo()?.roomType == RoomType.GROUPING_CLASS) {
                            ...
                            if (!isAttributeGot && initial == 1.0) {
                                AgoraLog?.e("$tag->ready getCoursewareAttribute")
                                getCoursewareAttribute()
                                isAttributeGot = true
                            }
                        }
