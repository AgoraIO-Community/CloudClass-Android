...
class AgoraUIVideoWindowDialog(info: EduContextUserDetailInfo, context: Context, private val optionListener: IAgoraOptionListener, private val eduContext: EduContextPool?) : Dialog(context, R.style.agora_dialog) {
    ...
    private fun initStyleDialog(anchor: View?) {
        ...
        if (eduContext?.userContext()?.getLocalUserInfo()?.userUuid == roleInfo.user.userUuid
                && roleInfo.user.role == AgoraEduContextUserRole.Teacher) { //本地老师，点了老师窗口
            ...
            recycler.layoutManager = GridLayoutManager(context, 3)
                ...
                && roleInfo.user.role == AgoraEduContextUserRole.Student) {//本地老师，点了学生窗口
            if (AgoraUIConfig.isLargeScreen) {
            ...
            recycler.layoutManager = GridLayoutManager(context, 5)
                ...
                && roleInfo.user.userUuid == eduContext?.userContext()?.getLocalUserInfo()?.userUuid) {
    ...
    private inner class OptionAdapter(private val recyclerView: RecyclerView) : RecyclerView.Adapter<OptionViewHolder>() {
        ...
        var iconsRes: Array<OptionItem> = when {
            eduContext?.userContext()?.getLocalUserInfo()?.userUuid == roleInfo.user.userUuid
                    && roleInfo.user.role == AgoraEduContextUserRole.Teacher -> {
                arrayOf(
            ...
            eduContext?.userContext()?.getLocalUserInfo()?.role == AgoraEduContextUserRole.Teacher
                    && roleInfo.user.role == AgoraEduContextUserRole.Student -> {
                arrayOf(
                        OptionItem(R.drawable.agora_option_icon_audio, "voice"),
                        OptionItem(R.drawable.agora_option_icon_video, "video"),
                        OptionItem(R.drawable.agora_option_icon_cohost, "cohost"),
        ...
        override fun onBindViewHolder(holder: OptionViewHolder, position: Int) {
            ...
            val itemWidth = if (eduContext?.userContext()?.getLocalUserInfo()?.userUuid == roleInfo.user.userUuid
                    && roleInfo.user.role == AgoraEduContextUserRole.Teacher) {
                recyclerView.width / 3
            } else if (roleInfo.user.role == AgoraEduContextUserRole.Student
                    && eduContext?.userContext()?.getLocalUserInfo()?.role == AgoraEduContextUserRole.Teacher) {
            ...
            when (item.itemName) {
                "voice" -> {
                    if (roleInfo.microState == io.agora.agoraeducore.core.context.EduContextDeviceState.UnAvailable
                            || roleInfo.microState == io.agora.agoraeducore.core.context.EduContextDeviceState.Closed) {
                        holder.icon.isEnabled = false
                        holder.icon.isSelected = false
                    } else {
                        holder.icon.isEnabled = true
                        holder.icon.isSelected = roleInfo.enableAudio
                    }
                }
                "video" -> {
                    if (roleInfo.cameraState == io.agora.agoraeducore.core.context.EduContextDeviceState.UnAvailable
                            || roleInfo.cameraState == io.agora.agoraeducore.core.context.EduContextDeviceState.Closed) {
                        holder.icon.isEnabled = false
                        holder.icon.isSelected = false
                    } else {
                        holder.icon.isEnabled = true
                        holder.icon.isSelected = roleInfo.enableVideo
                    }

                }
                "cohost" -> {
                    holder.icon.isActivated = !roleInfo.coHost
                }
                "grant" -> {
                    holder.icon.isActivated = !roleInfo.boardGranted
                ...
                when (item.itemName) {
                    "voice" -> {
                        if (holder.icon.isEnabled) {
                            optionListener.onAudioUpdated(roleInfo, !holder.icon.isSelected)
                    ...
                    "cohost" -> {
                        if (roleInfo.user.role != AgoraEduContextUserRole.Teacher) {
