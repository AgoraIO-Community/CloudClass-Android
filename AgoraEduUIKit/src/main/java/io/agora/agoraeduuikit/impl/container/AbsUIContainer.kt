...
abstract class AbsUIContainer(
    ...
    protected var largeWindowContainer: LinearLayout? = null
    ...
    private val widgetMessageObserver = object : AgoraWidgetMessageObserver {
        override fun onMessageReceived(msg: String, id: String) {
            ...
            if (id == WhiteBoard.id && packet.signal == BoardAudioMixingRequest) {
                (Gson().fromJson(Gson().toJson(packet.body), AgoraBoardAudioMixingRequestData::class.java))?.let {
                    when (it.type) {
                        AgoraBoardAudioMixingRequestType.Start -> {
                            eduContext?.mediaContext()?.startAudioMixing(it.filepath, it.loopback,
                                it.replace, it.cycle)
                        }
                        ...
                        AgoraBoardAudioMixingRequestType.SetPosition -> {
                            eduContext?.mediaContext()?.setAudioMixingPosition(it.position)
                        }
                    }
                } ?: Runnable {
...
enum class AgoraContainerType {
    OneToOne, SmallClass, SmallClassArt, LectureHall, LectureHallArt, Debug
}
