...
//            ?.startCoHostCarousel(interval = 10, count = 2, type = Sequence, condition = CameraOpened,
                callback = object : EduContextCallback<Unit> {
                    override fun onSuccess(target: Unit?) {
                        AgoraLog?.i("$tag->startCarousel success")
                    }

                    override fun onFailure(error: EduContextError?) {
                        AgoraLog?.e("$tag->startCarousel error: ${error?.let { GsonUtil.toJson(it) }}")
                        // restore image state
    ...
    private fun stopCarousel() {
        eduContext?.userContext()?.stopCoHostCarousel(object : EduContextCallback<Unit> {
            override fun onSuccess(target: Unit?) {
                AgoraLog?.i("$tag->stopCarousel success")
            }

            override fun onFailure(error: EduContextError?) {
                AgoraLog?.e("$tag->stopCarousel error: ${error?.let { GsonUtil.toJson(it) }}")
                // restore image state
