...
import com.agora.edu.component.helper.sort
...
class AgoraEduRosterComponent : AbsAgoraEduComponent {
    ...
    private var userListAdapter: AgoraEduRosterComponent.UserListAdapter? = null
    private var tvTeacherName: TextView? = null
    private var mConfig: AgoraEduCoreConfig? = null //获取launchConfig参数
    private var rosterType = EduContextRoomType.SmallClass
    private var role: AgoraEduContextUserRole? = AgoraEduContextUserRole.Student
    private val userService = RetrofitManager.instance().getService(AgoraEduSDK.baseUrl(), RosterService::class.java)
    ...
    private val uiDataProviderListener = object : UIDataProviderListenerImpl() {
        override fun onUserListChanged(userList: List<AgoraUIUserDetailInfo>) {
            ...
            if (rosterType == EduContextRoomType.LectureHall) {
                getStuListInRoster(curPageNo)
            }
        }
    }

    private fun getStuListInRoster(pageNo: Int) {
        userService.getUsersInRoster(mConfig?.appId!!, mConfig?.roomUuid!!, pageNo)
            .enqueue(RetrofitManager.Callback(0, object : ThrowableCallback<io.agora.agoraeducore.core.internal.edu.common.bean.ResponseBody<Data>> {
                override fun onFailure(throwable: Throwable?) {
                    Constants.AgoraLog?.e("$tag->${throwable?.let { GsonUtil.toJson(it) }}")
                }

                override fun onSuccess(res: io.agora.agoraeducore.core.internal.edu.common.bean.ResponseBody<Data>?) {
                    var stuList: MutableList<AgoraUIUserDetailInfo> = mutableListOf()
                    stuList.clear()
                    res?.data?.agoraEduRosterUserList?.forEach {
                        var role2 = if (it.role == "host") AgoraEduContextUserRole.Teacher else AgoraEduContextUserRole.Student
                        var hasAudio = false
                        var hasVideo = false
                        var streamUuid = ""
                    ...
                    Constants.AgoraLog?.e("$tag->${res?.let { GsonUtil.toJson(it) }}")
                }
            }))
    ...
    override fun initView(agoraUIProvider: IAgoraUIProvider) {
        ...
        if (mRoomType == EduContextRoomType.LectureHall) {
        ...
        recyclerView?.addOnScrollListener(object : RecyclerView.OnScrollListener() {
            override fun onScrollStateChanged(recyclerView: RecyclerView, newState: Int) {
                ...
                if (newState == RecyclerView.SCROLL_STATE_IDLE
                    ...
                    if (isOk) {
                        isOk = false
                        Constants.AgoraLog?.i("$tag->try to load more data")
                        getStuListInRoster(++curPageNo)
                    }
                }
    ...
    private fun setType(type: EduContextRoomType) {
        this.rosterType = type
    }

    private fun getLayoutRes(type: EduContextRoomType) = when (type) {
        EduContextRoomType.SmallClass -> R.layout.agora_userlist_dialog_layout
        EduContextRoomType.LectureHall -> R.layout.agora_userlist_dialog_layout
        else -> R.layout.agora_userlist_dialog_layout
    }
    ...
    private fun createItemViewHolder(
        ...
        return ClassUserHolder(
            LayoutInflater.from(parent.context)
                .inflate(R.layout.agora_userlist_dialog_list_item, null),
    ...
    private abstract inner class BaseUserHolder(
        private val type: EduContextRoomType,
        val view: View, val listener: UserItemClickListener
    ) : RecyclerView.ViewHolder(view) {
        abstract fun bind(item: AgoraUIUserDetailInfo)
    }

    private inner class UserListAdapter(val listener: UserItemClickListener) : ListAdapter<AgoraUIUserDetailInfo, BaseUserHolder>(UserListDiff()) {
    ...
    private inner class ClassUserHolder(
        ...
        private fun handleCameraState(
            ...
            if (item.isCoHost || rosterType == EduContextRoomType.LectureHall) {
...
internal interface RosterService {
    ...
    ): Call<io.agora.agoraeducore.core.internal.edu.common.bean.ResponseBody<Data>>
}
