...
import kotlinx.android.synthetic.main.fcr_countdown_clock_digit.view.*
import kotlinx.android.synthetic.main.fcr_view_simple_clock.view.*
import java.util.*
...
class AgoraTeachAidCountDownWidget : AgoraTeachAidMovableWidget() {
    ...
    fun getWidgetMsgObserver(): AgoraWidgetMessageObserver? {
        return countDownContent?.whiteBoardObserver
    }
    ...
    private inner class AgoraCountDownWidgetContent(val container: ViewGroup, val localUserInfo: AgoraWidgetUserInfo) {
        ...
        private val binding = AgoraCountdownWidgetContentBinding.inflate(LayoutInflater.from(container.context),
            container, true)
...
//                setDraggable(granted)
        ...
        fun parseProperties(properties: Map<String, Any>) {
            ...
            AgoraLog?.i("$tag->Countdown properties updated, started:" + started
            ...
            if (!initialState) {
                ...
                if (mCountdownStarted) {
                    ...
                    AgoraLog?.i("$tag->Countdown is reset, stop local countdown ticking and set initial state")
                    resetCountdownTicking()
                } else {
                    AgoraLog?.i("$tag->Countdown has been reset, resetCountdownTicking")
                    resetCountdownTicking()
                }
            }
        ...
        private fun startCountDownInSeconds(seconds: Long, remainder: Long) {
            AgoraLog?.i("$tag->startCountDown $seconds sec")
            var left = seconds
            if (left < 0) {
                left = 0
            ...
            if (binding.countdownClock.isAttachedToWindow) {
                binding.countdownClock.setCountdownListener(object : CountDownClock.CountdownCallBack {
                    override fun countdownAboutToFinish() {
                        AgoraLog?.i("$tag->Countdown is about to finish")
                        binding.countdownClock.setDigitTextColor(Color.RED)
                    }

                    override fun countdownFinished() {
                        AgoraLog?.i("$tag->Countdown finished")
                    }
                })
                ...
                AgoraLog?.i("$tag->[0]view has not attached to window, UI operations fail")
            }
        }
        ...
        private fun setSpecificCountdownTime(milliSeconds: Long) {
            if (binding.countdownClock.isAttachedToWindow) {
                ...
                AgoraLog?.i("$tag->[1]view has not attached to window, UI operations fail")
            }
        }
        ...
        private fun resetCountdownTicking() {
            if (binding.countdownClock.isAttachedToWindow) {
                ...
                AgoraLog?.i("$tag->[2]view has not attached to window, UI operations fail")
            }
        }
            ...
            binding.durationEditText.addTextChangedListener(object : TextWatcher {
                ...
                override fun afterTextChanged(s: Editable) {
                    if (!TextUtils.isEmpty(s.toString())) {
                        ...
                        binding.durationEditText.setTextColor(container.resources.getColor(
...
//                actionIsRestart = true
                binding.actionBtn.visibility = GONE
                binding.durationLayout.visibility = GONE
                val extraProperties = CountdownStatus(Started.value.toString(),
                    TimeUtil.currentTimeMillis().toString(), null, duration.toString()).convert()
                // active widget and upsert properties(sync for remoter)
                switchSelf(close = false, extraProperties = extraProperties)
            } else {
                AgoraLog?.e("$tag->duration is empty or null, please check.")
            }
        }
