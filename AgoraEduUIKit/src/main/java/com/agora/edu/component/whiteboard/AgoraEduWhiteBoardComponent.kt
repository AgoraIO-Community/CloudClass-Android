        ...
        ContextCompat.getMainExecutor(context).execute {
            ...
            whiteBoardWidget?.let {
                ...
                if (eduContext?.roomContext()?.getRoomInfo()?.roomType == RoomType.LARGE_CLASS) {
                    whiteBoardWidget?.widgetInfo?.roomProperties?.remove("grantedUsers") as? MutableMap<*, *>//移除本地授权用户
                    it.grantBoard(eduContext?.userContext()?.getLocalUserInfo()?.userUuid!!, false)//移除本地授权用户并同步到远端
                }
            }
