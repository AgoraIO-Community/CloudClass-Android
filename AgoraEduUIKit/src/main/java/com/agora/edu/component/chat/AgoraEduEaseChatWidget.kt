...
class AgoraEduEaseChatWidget : ChatPopupWidget(), InputMsgListener, ChatPagerListener {
    ...
    override fun init(container: ViewGroup) {
        ...
        LayoutInflater.from(mContext).inflate(com.hyphenate.easeim.R.layout.ease_chat_layout, null, false)?.let {
            layout = it
                ...
                mContext?.let {
                    AgoraUIToast.error(
                        context = it.applicationContext, text = mContext?.getString(
                            com.hyphenate.easeim.R.string.fcr_login_chat_failed
                        ) + "--" +
                                mContext?.getString(com.hyphenate.easeim.R.string.fcr_appKey_is_empty)
                    )
                }
            }
