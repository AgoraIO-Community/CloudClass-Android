    ...
    fun onMyTouchEvent(event: MotionEvent): Boolean {
        ...
        when (event.action) {
            MotionEvent.ACTION_DOWN -> {
                LogX.e(TAG, "child onTouchEvent = ACTION_DOWN")
            }
            MotionEvent.ACTION_MOVE -> {
                LogX.e(TAG, "child onTouchEvent = ACTION_MOVE :  ${event.x}")
            }

            MotionEvent.ACTION_UP -> {
                LogX.e(TAG, "child onTouchEvent = ACTION_UP")
                // only teacher can trigger floatingWindow
        ...
        info?.streamUuid?.let {
            if (isLocalStream(info.streamUuid)) {
                ...
                eduCore?.eduContextPool()?.mediaContext()
                    ?.subscribeLocalStream(info.streamUuid, !info.hasAudio, !info.hasVideo)
            }
        }
    }
...
//        }
    ...
    fun upsertUserDetailInfo(info: AgoraUIUserDetailInfo?, curVideoShouldRender: Boolean? = true) {
        ...
        ContextCompat.getMainExecutor(context).execute {
            ...
            LogX.e(TAG, "upsertUserDetailInfo curVideoShouldRender=$curVideoShouldRender || largeWindowOpened=$largeWindowOpened")
            //当前小窗不应该渲染流
            ...
            if (info != null) {
                ...
                if (!currentVideoOpen && newVideoOpen) {
                    LogX.e(TAG, "upsertUserDetailInfo info.streamUuid=${info.streamUuid}")
