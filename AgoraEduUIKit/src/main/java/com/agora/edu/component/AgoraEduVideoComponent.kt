 ...
 */
class AgoraEduVideoComponent : AbsAgoraEduComponent, OnClickListener, IAgoraOptionListener {
    ...
    override fun initView(agoraUIProvider: IAgoraUIProvider) {
        ...
        val gestureDetector = GestureDetector(context, object : GestureDetector.SimpleOnGestureListener() {
            override fun onSingleTapConfirmed(e: MotionEvent): Boolean {
                ...
                if (localUsrInfo?.role != Teacher || eduContext?.roomContext()
                        ?.getRoomInfo()?.roomType == RoomType.ONE_ON_ONE && curUserDetailInfo?.role == Teacher
                ) {
                    return super.onSingleTapConfirmed(e)
                }
                m = curUserDetailInfo?.let {
    ...
    fun setPublishAudioVideo(info: AgoraUIUserDetailInfo?) {
        info?.streamUuid?.let {
            if(isLocalStream(info.streamUuid)) {
                AgoraLog?.e("$tag->upsertUserDetailInfo audio = ${!info.hasAudio} || video = ${!info.hasVideo}")
                eduCore?.eduContextPool()?.mediaContext()?.subscribeLocalStream(info.streamUuid, !info.hasAudio, !info.hasVideo)
            }
        }
    }
...
//        }
    ...
    fun upsertUserDetailInfo(info: AgoraUIUserDetailInfo?, curVideoShouldRender: Boolean? = true) {
        ...
        AgoraLog?.e("$tag->upsertUserDetailInfo:${info.toString()}")
        ContextCompat.getMainExecutor(context).execute {
            ...
            AgoraLog?.e("$tag->upsertUserDetailInfo curVideoShouldRender=$curVideoShouldRender || largeWindowOpened=$largeWindowOpened")

            //当前小窗不应该渲染流
            if (curVideoShouldRender == false || largeWindowOpened) {// curVideoShouldRender:  current video item should render
                //large window showed
                binding.audioIcContainer.visibility = GONE
                binding.audioIc.visibility = GONE
            ...
            if (info != null) {
                ...
                if (!currentVideoOpen && newVideoOpen) {
                    AgoraLog?.e("$tag->upsertUserDetailInfo info.streamUuid=${info.streamUuid}")
                    ...
                    videoListener?.onRendererContainer(parent, info)
                    AgoraLog?.e("$tag->upsertUserDetailInfo info.streamUuid=${info.streamUuid}")
                }
            } else {
