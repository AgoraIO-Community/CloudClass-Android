...
internal class MyCloudManager(val appId: String, val userUuid: String) {
    ...
    fun fetchCoursewareWithPage(resourceName: String? = null, page: Int, pageSize: Int = defaultPageSize, callback: EduContextCallback<MyCloudCoursewareRes>) {
        service.fetchCoursewares(
            ...
            .enqueue(RetrofitManager.Callback(0, object : ThrowableCallback<ResponseBody<MyCloudCoursewareRes>> {
                ...
                override fun onFailure(throwable: Throwable?) {
                    AgoraLog?.e("tag->fetchCoursewareWithPage-failed:${throwable?.let { GsonUtil.toJson(throwable) }}")
