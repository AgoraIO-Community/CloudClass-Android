...
import com.agora.edu.component.common.*
...
class AgoraEduHeadComponent : AbsAgoraEduConfigComponent<FcrStateBarUIConfig> {
    ...
    private var curNetworkState = EduContextNetworkQuality.Good
    ...
    lateinit var networkComponent: FcrNetworkPopupComponent
    ...
    override fun initView(agoraUIProvider: IAgoraUIProvider) {
        ...
        networkComponent = FcrNetworkPopupComponent(binding.root.context)
        networkComponent.initView(agoraUIProvider)

        binding.agoraStatusBarNetworkStateIcon.setOnClickListener {
            if (networkComponent.isShow()) {
                networkComponent.dismiss()
            } else {
                networkComponent.showAsDropDown(it)
            }
        }
    }
...
//    }
    ...
    var stateQuality: EduContextNetworkQuality = EduContextNetworkQuality.Good
    private fun updateNetworkState(state: EduContextNetworkQuality) {
        ContextCompat.getMainExecutor(context).execute {
            binding.agoraStatusBarNetworkStateIcon.setCompoundDrawablesWithIntrinsicBounds(
                FcrNetWorkUtils.getNetworkStateIcon(context, state), null, null, null)
            binding.agoraStatusBarNetworkStateIcon.setTextColor(FcrNetWorkUtils.getNetWorkStateColor(context, state))
            binding.agoraStatusBarNetworkStateIcon.setText(FcrNetWorkUtils.getNetWorkStateDesc(state))

            if(state == EduContextNetworkQuality.Down && stateQuality != state){
                AgoraUIToast.error(context, text = context.resources.getString(R.string.fcr_monitor_network_disconnected))
            }
            stateQuality = state
        }

//        if (state == EduContextNetworkQuality.Bad && badCount == badNetworkThreshold) {
//            AgoraUIToast.warn(context, text = context.resources.getString(R.string.fcr_monitor_network_poor))
//            badCount = 0
