...
class AgoraEduHandComponent : AbsAgoraEduComponent, AgoraUIHandsWaveCountDownListener {
    ...
    private var binding: AgoraEduHandComponentBinding = AgoraEduHandComponentBinding.inflate(LayoutInflater.from(context), this, true)
    ...
    private val touchDownTimer: HandsWaveTimer = HandsWaveTimer(handsWaveTimeout, timerTick,
        object : ((Int) -> Unit) {
    ...
    private val touchCancelTimer = HandsWaveTimer(handsWaveTimeout, timerTick, object : ((Int) -> Unit) {
        override fun invoke(timeout: Int) {
            handleTimerStart(timeout)
        }
    },
...
//        )
            ...
            popupView!!.popupWindow?.showAsDropDown(
                binding.optionItemHandsup,
                xOffset,
                -binding.optionItemHandsup.height / 4 * 3,
    ...
    override fun onCountDownStart(timeoutInSeconds: Int) {
        Constants.AgoraLog?.d("$tag->onCountDownStart, timeout $timeoutInSeconds seconds")
        post {
    ...
    override fun onCountDownTick(secondsToFinish: Int) {
        Constants.AgoraLog?.d("$tag->onCountDownTick, $secondsToFinish seconds remaining")
        post {
    ...
    override fun onCountDownEnd() {
        Constants.AgoraLog?.d("$tag->onCountDownEnd")
        post {
