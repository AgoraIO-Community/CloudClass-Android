...
import com.agora.edu.component.teachaids.vote.VoteState.Init
import com.agora.edu.component.teachaids.vote.VoteState.Submitted
import com.agora.edu.component.teachaids.vote.VoteState.End
import com.agora.edu.component.teachaids.vote.VoteState.Polling
...
class AgoraTeachAidVoteWidget : AgoraTeachAidMovableWidget() {
    ...
    fun getWidgetMsgObserver(): AgoraWidgetMessageObserver? {
        return voteContent?.whiteBoardObserver
    }
    ...
    internal inner class AgoraVoteWidgetContent(val container: ViewGroup, val localUserInfo: AgoraWidgetUserInfo) {
        ...
        private val binding = AgoraVoteWidgetContentBinding.inflate(LayoutInflater.from(container.context), container,
            true)
...
//                setDraggable(granted)
        ...
        init {
            binding.voteChoices.layoutManager = VoteLinearLayoutManager(container.context,
            object : VoteLinearLayoutManager.OnLayoutListener {
                override fun onLayoutCompleted() {
                    val packet = AgoraTeachAidWidgetInteractionPacket(NeedRelayout, Unit)
        ...
        fun parseProperties(roomProperties: Map<String, Any>, userProperties: Map<String, Any>?) {
            ...
            if (roomProperties.isNullOrEmpty()) {
                AgoraLog?.e("$tag->cur properties is empty, please check!")
                return
            }
            val voteDataJson = GsonUtil.toJson(roomProperties)
            if (voteDataJson.isNullOrEmpty()) {
                AgoraLog?.e("$tag->cur properties json is empty, please check!")
                return
            }
            voteData = GsonUtil.jsonToObject<VoteData>(voteDataJson)
            if (voteData == null) {
                AgoraLog?.e("$tag->vote transform failed, please check!")
                return
            }
            ...
            voteData?.let {
                widgetInfo?.localUserInfo?.let { info ->
                    ...
                    if (selectIndex.isNotEmpty()) {
                        ...
                        AgoraLog?.w("$tag->selectIndex is empty, please select item.")
                    }
                }
            }
            mStudentState = Submitted.value
        ...
        private fun onTeacherSwitchSelf(close: Boolean = true, extraProperties: Map<String, Any>? = null) {
            val packet = AgoraTeachAidWidgetInteractionPacket(ActiveState,
...
// list pollItems for student to select
internal class ChoiceAdapter(private var pollingItems: List<String>, private var multi: Boolean) :
    ...
    override fun onCreateViewHolder(viewGroup: ViewGroup, viewType: Int): ViewHolder {
        val view = LayoutInflater.from(viewGroup.context)
            .inflate(R.layout.fcr_vote_choice_item, viewGroup, false)
...
internal class ResultAdapter(private val voteData: VoteData) :
    ...
    override fun onCreateViewHolder(viewGroup: ViewGroup, viewType: Int): ViewHolder {
        val view = LayoutInflater.from(viewGroup.context)
            .inflate(R.layout.fcr_vote_result_item, viewGroup, false)

        return ViewHolder(view)
    }

    override fun onBindViewHolder(viewHolder: ViewHolder, position: Int) {
        val text = String.format(viewHolder.itemView.resources.getString(R.string.fcr_votetitle), position + 1,
...
internal class BuildChoiceAdapter(val choiceList: MutableList<String?>) :
    ...
    class ViewHolder(val binding: AgoraWidgetVoteBuildChoiceLayoutBinding) : RecyclerView.ViewHolder(binding.root)
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        val binding = AgoraWidgetVoteBuildChoiceLayoutBinding.inflate(LayoutInflater.from(parent.context), parent,
            false)
...
internal class VoteManager(val appId: String, val roomUuid: String) {
    ...
    private val voteService = RetrofitManager.instance().getService(AgoraEduSDK.baseUrl(), VoteService::class.java)
    ...
    fun startVote(mode: Int, pollingTitle: String, pollingItems: MutableList<String>) {
        ...
        voteService.start(appId, roomUuid, body)
