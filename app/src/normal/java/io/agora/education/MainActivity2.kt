...
class MainActivity2 : AppCompatActivity(), View.OnClickListener {
    ...
    private lateinit var edVideoStreamMode: AppCompatEditText
        ...
        ConfigUtil.config(ConfigUtil.getRegionUrl(roomRegion), userUuid, object : EduCallback<ConfigData> {
            override fun onSuccess(res: ConfigData?) {
                ...
                res?.let {
                    ...
                    var videoStreamMode: Int? = edVideoStreamMode.text.toString().toIntOrNull()
                    val config: AgoraEduLaunchConfig
                    if (videoStreamKey == "" || videoStreamMode == null) {
                        config = AgoraEduLaunchConfig(userName, userUuid, roomName, roomUuid,
                                roleType, roomType, it.rtmToken, startTime, duration, roomRegion, null)
                    } else {
                        config = AgoraEduLaunchConfig(userName, userUuid, roomName, roomUuid,
                                roleType, roomType, it.rtmToken, startTime, duration, roomRegion, AgoraEduMediaOptions(AgoraEduMediaEncryptionConfigs(videoStreamKey, videoStreamMode)))
                    }
                    config.appId = it.appId

                    // Set global app id record for further use
                    EduApplication.setAppId(it.appId)
                    AgoraEduSDK.setConfig(AgoraEduSDKConfig(it.appId, 0))
    ...
    private fun launch(config: AgoraEduLaunchConfig) {
        AgoraEduSDK.launch(this, config) { state: AgoraEduEvent ->
