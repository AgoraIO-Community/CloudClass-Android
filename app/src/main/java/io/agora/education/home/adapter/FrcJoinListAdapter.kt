...
class FrcJoinListAdapter : FcrPreloadAdapter<FrcJoinListAdapter.FrcJoinListHolder>() {
    ...
    override fun onBindViewHolder(holder: FrcJoinListHolder, position: Int) {
        holder.bindView(position, list[position], onEnterRoomListener)
    }
    ...
    class FrcJoinListHolder(var binding: ItemJoinClassStateBinding) : RecyclerView.ViewHolder(binding.root) {
        ...
        fun bindView(position: Int, info: FcrRoomDetail, onEnterRoomListener: ((FcrRoomDetail) -> Unit)?) {
            ...
            binding.tvJoinClassTime.text = AppUtil.getRoomDate(info.startTime, info.endTime)
            ...
            binding.ivJoinClassShare.setOnClickListener {
                // 分享链接
                FcrShareDialog.newInstance(context).setLinkUrl(info.getShareLink(context)).show()
            }
