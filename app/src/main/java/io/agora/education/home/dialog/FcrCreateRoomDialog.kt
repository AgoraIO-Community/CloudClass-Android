...
class FcrCreateRoomDialog(context: Context) : FcrBaseDialog(context) {
    ...
    override fun initView() {
        ...
        if (!TextUtils.isEmpty(defName)) {
            ...
            binding.etJoinRoomName.setText(roomName)
            binding.etJoinRoomName.setSelection(length)
        }
...
//        binding.llMoreSettingRoot.visibility = GONE
    ...
    fun createRoom() {
        val roomName = binding.etJoinRoomName.text.toString()
        if (roomName.isEmpty()) {
            binding.etJoinRoomName.startAnimation(AnimationUtils.loadAnimation(context, R.anim.fcr_input_shake))
            binding.scrollContent.fullScroll(NestedScrollView.FOCUS_UP)
            ToastManager.showShort(R.string.fcr_create_label_roomname_empty)
            return
        }
        if (startTime == 0L) {
        ...
        if (startTime < System.currentTimeMillis()) {
            ToastManager.showShort(R.string.fcr_create_tips_starttime)
            return
        }
        loading.show()
