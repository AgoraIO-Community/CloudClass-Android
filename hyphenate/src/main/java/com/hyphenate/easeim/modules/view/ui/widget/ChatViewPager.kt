...
class ChatViewPager(context: Context, attributeSet: AttributeSet?, defStyleAttr: Int) : LinearLayout(context, attributeSet, defStyleAttr), EMMessageListener, EMChatRoomChangeListener, ViewClickListener, EMConnectionListener {
    ...
    fun initView() {
        ...
        val titleList = listOf<String>(
                context.getString(R.string.fcr_chat),
                context.getString(R.string.fcr_announcement),
        )
...
//        viewPager.offscreenPageLimit = 2
    ...
    private fun chooseFirstTab() {
        ...
        title?.setTextColor(ContextCompat.getColor(context, R.color.fcr_blue))
    ...
    fun loginIM() {
        ...
        EMClient.getInstance().login(userName, userUuid, object : EMCallBack {
            ...
            override fun onError(code: Int, error: String) {
                ...
                if (loginLimit == 2) {
                    ThreadManager.instance.runOnMainThread {
                        Toast.makeText(context, context.getString(R.string.fcr_login_chat_failed)+":$code:$error", Toast.LENGTH_SHORT).show()
                    }
                    return
                }
    ...
    private fun joinChatRoom() {
        ...
        EMClient.getInstance().chatroomManager().joinChatRoom(chatRoomId, object : EMValueCallBack<EMChatRoom?> {
            ...
            override fun onError(error: Int, errorMsg: String) {
                ...
                if (joinLimit == 2) {
                    ThreadManager.instance.runOnMainThread {
                        Toast.makeText(context, context.getString(R.string.fcr_login_chat_failed)+"--"+context.getString(R.string.fcr_join_chat_room_failed)+":$error:$errorMsg", Toast.LENGTH_SHORT).show()
                    }
                    return
                }
                joinChatRoom()
