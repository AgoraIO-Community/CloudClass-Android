...
import io.agora.online.easeim.view.adapter.ChatViewPagerAdapter
import io.agora.online.easeim.view.`interface`.ChatPagerListener
...
class ChatViewPager(context: Context, attributeSet: AttributeSet?, defStyleAttr: Int) :
    LinearLayout(context, attributeSet, defStyleAttr), MessageListener, ChatRoomChangeListener,
    ViewEventListener, ConnectionListener {
    ...
    private var mCurrentPrivateReceiver: AgoraUIUserDetailInfo? = null

    init {
        LayoutInflater.from(context).inflate(R.layout.fcr_online_chat_total_layout, this)
    ...
    private fun getTabView(context: Context, title: String): View {
        val view: View =
            LayoutInflater.from(context).inflate(R.layout.fcr_online_view_pager_tab_item_layout, null)
    ...
    override fun onMessageReceived(messages: MutableList<ChatMessage>?) {
        messages?.let {
            for (message in messages) {
                val type = message.getIntAttribute(
                    EaseConstant.MSG_TYPE,
                    EaseConstant.NORMAL_MSG
                )

                if (type == EaseConstant.NORMAL_MSG
                ) {
            ...
            if (message.chatType == ChatMessage.ChatType.ChatRoom && message.to == chatRoomId) {
                ...
                when (body.action()) {
                    EaseConstant.SET_ALL_MUTE, EaseConstant.REMOVE_ALL_MUTE -> {
                        notifyBody.params =
                            mutableMapOf(Pair(EaseConstant.OPERATION, body.action()))
                    }
                ...
                notifyMessage.setAttribute(
                    EaseConstant.NICK_NAME,
                    message.getStringAttribute(EaseConstant.NICK_NAME, message.from)
                )
        ...
        call.enqueue(object : Callback {
            override fun onFailure(call: Call, e: IOException) {
                ...
                ThreadManager.instance.runOnMainThread {
                    Toast.makeText(
                        context,
                        context.getString(R.string.fcr_hyphenate_im_fetch_token_failed) + ":${e.message}",
                        Toast.LENGTH_SHORT
                    ).show()
                }
            }
                ...
                ThreadManager.instance.runOnMainThread {
                    if (code == 200 && !repBody.isNullOrEmpty()) {
                        try {
                            ...
                            Toast.makeText(
                                context,
                                context.getString(R.string.fcr_hyphenate_im_fetch_token_failed) + ": $code : ${e.message}",
                                Toast.LENGTH_SHORT
                            ).show()
                        }
                    } else {
                        Toast.makeText(
                            context,
                            context.getString(R.string.fcr_hyphenate_im_fetch_token_failed) + ": $code : $repBody",
                            Toast.LENGTH_SHORT
                        ).show()
                    }
                }
    ...
    private fun loginIM() {
        ...
        ChatClient.getInstance().loginWithToken(userName, imToken, object : CallBack {
            override fun onSuccess() {
                ...
                extJson.put(EaseConstant.ROLE, role)
                extJson.put(EaseConstant.USER_UUID, userUuid)
            ...
            override fun onError(code: Int, error: String) {
                ...
                if (loginLimit == 2) {
                    ThreadManager.instance.runOnMainThread {
                        Toast.makeText(
                            context,
                            context.getString(R.string.fcr_hyphenate_im_login_chat_failed) + ":$code:$error",
                            Toast.LENGTH_SHORT
                        ).show()
                    }
                    return
                ...
                override fun onError(error: Int, errorMsg: String) {
                    ...
                    if (joinLimit == 2) {
                        ThreadManager.instance.runOnMainThread {
                            Toast.makeText(
                                context,
                                context.getString(R.string.fcr_hyphenate_im_login_chat_failed) + "--" + context.getString(
                                    R.string.fcr_hyphenate_im_join_chat_room_failed
                                ) + ":$error:$errorMsg",
                                Toast.LENGTH_SHORT
                            ).show()
                        }
    ...
    override fun onMsgReceiverClick() {
        chatPagerListener?.onMsgReceiverClick()
    }

    override fun onPrivateReceiverClick(receiver: AgoraUIUserDetailInfo?) {
        mCurrentPrivateReceiver = receiver
        chatPagerListener?.onPrivateReceiverClick(receiver)
    }
    ...
    private fun showOuterLayerUnread() {
        ...
        val noticeUnread =
            tabLayout?.getTabAt(announcementTab)?.view?.findViewById<ImageView>(R.id.iv_tips)
    ...
    @UiThread
    fun setPrivateChatViewDisplayed(user: AgoraUIUserDetailInfo?, displayed: Boolean) {
        chatView.showPrivateChatView(user)

        if (displayed) {
            chatView.viewEventListener?.onPrivateReceiverClick(user)
            //chatView.showPrivateChatView(user)
        } else {
            chatView.viewEventListener?.onPrivateReceiverClick(null)
            //chatView.hidePrivateChatView()
        }
    }

    fun sendTextMessage(content: String) {
        if (mCurrentPrivateReceiver != null && mCurrentPrivateReceiver!!.userUuid.isNotEmpty()) {
            sendTextMessageTo(content, mCurrentPrivateReceiver!!.userUuid)
            return
        }
    ...
    fun sendImageMessage(uri: Uri) {
        if (mCurrentPrivateReceiver != null && mCurrentPrivateReceiver!!.userUuid.isNotEmpty()) {
            sendImageMessageTo(uri, mCurrentPrivateReceiver!!.userUuid)
            return
        }
    ...
    private fun sendTextMessageTo(content: String, receiver: String) {
        val message = ChatMessage.createTextSendMessage(content, chatRoomId)
        val list: ArrayList<String> = ArrayList()
        list.add(receiver)
        message.setReceiverList(list)

        sendMessage(message)
    }

    private fun sendImageMessageTo(uri: Uri, receiver: String) {
        val message = ChatMessage.createImageSendMessage(uri, false, chatRoomId)
        val list: ArrayList<String> = ArrayList()
        list.add(receiver)
        message.setReceiverList(list)
    ...
    private fun sendMessage(message: ChatMessage) {
        if (!(EaseRepository.instance.isInit && EaseRepository.instance.isLogin)) {
            Toast.makeText(
                context,
                context.getString(R.string.fcr_hyphenate_im_send_message_failed) + ":" + context.getString(
        ...
        setExtBeforeSend(message, EaseConstant.NORMAL_MSG)
    ...
    private fun setExtBeforeSend(message: ChatMessage, type: Int) {
        message.setAttribute(EaseConstant.ROLE, EaseConstant.ROLE_STUDENT)
        message.setAttribute(EaseConstant.MSG_TYPE, type)
        message.setAttribute(EaseConstant.ROOM_UUID, roomUuid)
        message.setAttribute(EaseConstant.NICK_NAME, nickName)

        if (mCurrentPrivateReceiver != null) {
            val receiverList = mutableListOf<AgoraIMUserInfo>()
            receiverList.add(
                AgoraIMUserInfo(
                    mCurrentPrivateReceiver?.userUuid!!,
                    mCurrentPrivateReceiver?.userName!!,
                    ""
                )
            )

            var json = JSONArray(GsonUtil.gson.toJson(receiverList))

            message.setAttribute(EaseConstant.RECEIVER_LIST, json)
        }
