package io.agora.online.provider
...
class UIDataProvider(private var eduCore: AgoraEduCore?, private val eduContext: EduContextPool?) {
    ...
    fun setGrantedUsers(eduCore: AgoraEduCore?) {
        try {
            val map =
                (((eduCore?.room()?.roomProperties?.get("widgets") as? Map<*, *>)?.get("netlessBoard") as? Map<*, *>)?.get(
                    "extra"
                ) as? Map<*, *>)?.get(
                    "grantedUsers"
        ...
        override fun onMessageReceived(msg: String, id: String) {
            ...
            if (id == AgoraWidgetDefaultId.WhiteBoard.id && packet.signal == AgoraBoardInteractionSignal.BoardGrantDataChanged) {
                ...
                if (packet.body is MutableList<*>) { // 白板开关的格式
                    ...
                    val agoraBoard =
                        GsonUtil.gson.fromJson(bodyStr, AgoraBoardGrantData::class.java)
...
//                }
    ...
    val groupHandler = object : FCRGroupHandler(){
        override fun onGroupInfoUpdated(groupInfo: FCREduContextGroupInfo) {
            ...
            LogX.e("UIData","onGroupInfoUpdated 分组：$groupInfo.state")
        }

        override fun onAllGroupUpdated(groupInfo: FCRAllGroupsInfo) {
            LogX.e("UIData", "onAllGroupUpdated groupList=$groupInfo")
            if (groupInfo.details == null) {
                FcrGroupUserManager.clearGroupList()
            } else {
                for ((key, value) in groupInfo.details) {
                    value.users.forEach {
                        FcrGroupUserManager.setGroupUser(key, it.userUuid)
                    }
                }
            }
//            groupInfo.details?.let { map ->
...
//            }
        ...
        override fun onSubRoomListRemoved(subRoomList: List<AgoraEduContextSubRoomInfo>) {
            //LogX.e("UIData", "onSubRoomListRemoved groupList=$subRoomList")
            subRoomList.forEach {
                FcrGroupUserManager.removeGroup(it.subRoomUuid)
            }
        }

        override fun onUserListAddedToSubRoom(
            subRoomList: List<String>,
            subRoomUuid: String,
            user: EduBaseUserInfo?
        ) {
            subRoomList.forEach {
                FcrGroupUserManager.setGroupUser(subRoomUuid, it)
            }
            //LogX.e("UIData","onUserListAddedToSubRoom groupList=$subRoomList")
        }
        ...
        override fun onUserMovedToSubRoom(
            ...
            FcrGroupUserManager.removeGroupUser(fromSubRoomUuid, userUuid)
            FcrGroupUserManager.setGroupUser(toSubRoomUuid, userUuid)
            //LogX.e("UIData","onUserMovedToSubRoom groupList=$userUuid")
        }
        ...
        override fun onUserListRemovedFromSubRoom(
            ...
            subRoomUuid?.let {
                userList.forEach {
                    FcrGroupUserManager.removeGroupUser(subRoomUuid, it.userUuid)
                }
            }
            //LogX.e("UIData", "onUserListRemovedFromSubRoom groupList=$userList")
        }
    }
    ...
    init {
        eduContext?.widgetContext()
            ?.addWidgetMessageObserver(widgetObserver, AgoraWidgetDefaultId.WhiteBoard.id)
        eduContext?.widgetContext()
            ?.addWidgetMessageObserver(widgetObserver, AgoraWidgetDefaultId.LargeWindow.id)
...
//        }
    ...
    private fun callbackCoHostUserList(
        ...
        //LogX.e("UIData", "groupUserList all >>>>>>>> ${eduCore?.room()?.roomProperties?.get(PropertyData.groupKey)}")
        //LogX.e("UIData", "FcrGroupUserManager groupUserList >>>>>>>> ${FcrGroupUserManager.getGroupUserList()}")

        val list = mutableListOf<AgoraEduContextUserInfo>()
            val userUuid = eduContext?.userContext()?.getLocalUserInfo()?.userUuid
            eduContext?.streamContext()?.getAllStreamList()?.forEach {
                // 老师和分组里面的人不用显示
                if (it.owner.role != AgoraEduContextUserRole.Teacher) {
                    if (eduCore?.eduContextPool()?.roomContext()?.getRoomInfo()?.roomType == RoomType.GROUPING_CLASS) {
                        list.add(it.owner)
                    } else {
                        if (it.owner.userUuid == userUuid || !FcrGroupUserManager.getGroupUserList().contains(it.owner.userUuid)) {
                            list.add(it.owner)
                        }
                    }
                }
...
data class AgoraIMUserInfo(
    val userId: String,
    val nickName: String,
    val avatarUrl: String
) {
    fun copy(): AgoraIMUserInfo {
        return AgoraIMUserInfo(
