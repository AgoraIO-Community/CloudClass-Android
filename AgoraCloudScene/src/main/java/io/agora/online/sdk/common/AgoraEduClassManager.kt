...
class AgoraEduClassManager(var context: AgoraBaseClassActivity, var eduCore: AgoraEduCore?) {
    ...
    fun launchSubRoom(
        ...
        synchronized(lockObject) {
            val launchConfig = AgoraOnlineClassroomSDK.getCurrentLaunchConfig()
            ...
            launchConfig?.let {
                ...
                AgoraOnlineClassroomSDK.launch2(context, launch) {
                    //fullLoading.dismiss()
    ...
    fun showLeaveGroupRoom(callback: (Boolean) -> Unit) {
        ...
        context.let {
            val customView = LayoutInflater.from(it).inflate(R.layout.fcr_online_grouping_leave_layout, null)
