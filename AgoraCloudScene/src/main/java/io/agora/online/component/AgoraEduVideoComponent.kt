...
import io.agora.online.databinding.FcrOnlineEduVideoComponentBinding
import io.agora.online.impl.video.AgoraEduFloatingControlWindow
...
import io.agora.online.impl.video.IAgoraOptionListener
 ...
 */
class AgoraEduVideoComponent : AbsAgoraEduComponent, IAgoraOptionListener,
    FcrDragTouchGroupView.OnDoubleClickListener {
    ...
    var largeWindowOpened: Boolean = false // 当前视频组件对应的大窗 widget 是否打开
    var isLargeWindow: Boolean = false // 当前组件是否是大窗组件还是基础视频窗组件，用于拖拉事件
    private var lastVideoRender: Boolean? = true
    private val binding = FcrOnlineEduVideoComponentBinding.inflate(LayoutInflater.from(context), this, true)
    var videoViewParent: ViewGroup = binding.videoContainer
    private val dash = "-"
    private var defaultPositionPercent = 0.5F // 默认位置 x，y

    // private var curZIndex =0 // 当前 video 对应大窗的 zIndex
    ...
    private var lastX: Int = 0 // actiondown 的时候记录坐标x
    private var lastY: Int = 0
    private var curX: Int = 0
    private var curY: Int = 0
    private var agoraLargeWindowContainer: View? = null
    private var listItemLeft = 0f // 用于学生列表中, 当前 item 的 left 值
    ...
    fun handleClick() {
        ...
        if ((localUsrInfo?.role != Teacher
                    && localUsrInfo?.role != Student) || (eduContext?.roomContext()
                ?.getRoomInfo()?.roomType == RoomType.ONE_ON_ONE
                    && curUserDetailInfo?.role == Teacher)
        ) {
            // 如果不是老师以及学生(主要私聊功能)，或者是 1v1 的老师 直接返回
            return
        }
...
//                extraProperties["zIndex"] = curMaxZIndex
                ...
                if (eduContext?.userContext()?.getLocalUserInfo()?.role == Teacher) {
                    when (eduContext?.roomContext()?.getRoomInfo()?.roomType) {
                        RoomType.SMALL_CLASS -> {
                            LogX.e(
                                TAG, "child onTouchEvent event.y: ${event.y} agora_small_video_h: ${
                                    resources.getDimension(R.dimen.agora_small_video_h)
                                }"
                            )

                            if (event.y > resources.getDimension(R.dimen.agora_small_video_h) * 2) {
                                // 如果大窗没有打开过，则发送 widget active 打开大窗
                                eduContext?.roomContext()?.getRoomInfo()?.roomUuid?.let {
                                    isLargeWindow = FCRLargeWindowManager.isLargeWindow(
                                        it,
                                        curUserDetailInfo?.streamUuid!!
                                    ) // 判断当前组件是否属于大窗
                                }
                                if (!isLargeWindow) { // 如果不是大窗，则打开大窗widget
                                    LogX.e(TAG, "child onTouchEvent 打开大窗 x = ${event.x}")
                                    // 大窗的宽
                                    val largeWindowWidth =
                                        defaultSizeWidthPercentSmall * agoraLargeWindowContainer!!.width
                                    // 大窗的高
                                    val largeWindowHeight = largeWindowWidth * 0.62f
                                    val medWidth =
                                        agoraLargeWindowContainer!!.width - largeWindowWidth
                                    val medHeight =
                                        agoraLargeWindowContainer!!.height - largeWindowHeight
                                    // widget 的左边界的值除以 widget 有效移动范围宽(medWidth)
                                    // 当前 view 距离父布局的距离(left) - 大窗宽的一半 - event.x 绝对值
                                    var curPositionPercentX = 0f
                                    // widget 的上边界的值除以 widget 有效移动范围高(medHeight)
                                    var curPositionPercentY =
                                        (event.y - largeWindowHeight) / medHeight
                                    if (event.x < 0) {
                                        // 往当前 view 左边滑动
                                        if (curUserDetailInfo?.role == Student) {
                                            ...
                                            LogX.e(
                                                TAG,
                                                "child onTouchEvent 打开大窗 left = $listItemLeft event.x = ${event.x} largeWindowWidth / 2: ${largeWindowWidth / 2}"
                                            )
                                            curPositionPercentX =
                                                (listItemLeft + event.x - largeWindowWidth / 2) / medWidth
                                        } else {
                                            // 移动讲台区域老师视频窗
                                            curPositionPercentX =
                                                (left - largeWindowWidth / 2 - abs(event.x)) / medWidth
                                        }
                                    } else {
                                        // 往当前 view 右边滑动
                                        LogX.e(
                                        ...
                                        if (curUserDetailInfo?.role == Student) {
                                            ...
                                            curPositionPercentX =
                                                (event.x + largeWindowWidth / 2) / medWidth
                                        } else {
                                            curPositionPercentX =
                                                (left + event.x - largeWindowWidth / 2) / medWidth
                                        }
                                    }
                                    // x 轴滑动到最左边的情况
                                    ...
                                    // 准备发送 widget active 消息
                                    val extraProperties: MutableMap<String, Any> = mutableMapOf()
                                    curMaxZIndex += 1
                                    extraProperties["zIndex"] = curMaxZIndex
                                    extraProperties[StaticData.extraUserUuidKey] =
                                        curUserDetailInfo?.userUuid!!
                                    eduContext?.widgetContext()?.setWidgetActive(
                                        widgetId = AgoraWidgetDefaultId.LargeWindow.id + dash + curUserDetailInfo?.streamUuid,
                                        ownerUserUuid = curUserDetailInfo?.userUuid,
                                        roomProperties = extraProperties,
                        ...
                        RoomType.LARGE_CLASS, RoomType.ONE_ON_ONE -> {
                            if (event.x <= 0 && !isLargeWindow) { // 如果大窗没有打开过，则发送 widget active 打开大窗
                                LogX.e(TAG, "child onTouchEvent 打开大窗 x = ${event.x}")

                                val largeWindowWidth =
                                    defaultSizeWidthPercent * agoraLargeWindowContainer!!.width
                                ...
                                var curPositionPercentX =
                                    (agoraLargeWindowContainer!!.width - largeWindowWidth / 2 - abs(
                                        event.x
                                    )) / medWidth
                                var curPositionPercentY =
                                    (event.y - largeWindowHeight / 2) / medHeight
                                // x 轴滑动到最左边的情况
                                if (curPositionPercentX < 0) {
                                    curPositionPercentX = 0f
                                }
                                // x 轴滑动到接近最右边边的情况:滑动到的位置(大窗容器宽度 - x 偏移的绝对值) + 一半的宽度 > 大窗容器宽度
                                if (agoraLargeWindowContainer!!.width - largeWindowWidth / 2 - abs(
                                        event.x
                                    ) + largeWindowWidth > agoraLargeWindowContainer!!.width
                                ) {
                                    curPositionPercentX =
    ...
    fun setPublishAudioVideo(info: AgoraUIUserDetailInfo?) {
        info?.streamUuid?.let {
            if (isLocalStream(info.streamUuid)) {
                ...
                eduCore?.eduContextPool()?.streamContext()?.muteLocalStream(info.streamUuid, !info.hasAudio, !info.hasVideo)
            }
        }
    }
        ...
        if (binding.audioIc.isEnabled && binding.audioIc.isSelected && value > 0 && !gifRunning) {
            ContextCompat.getMainExecutor(context).execute {
                ...
                if (value > 40) { // value > 40
                    ...
                    if (isBackgroundGreen) { // 防止上次设置了 false 再次进来后多次发送 postDelayed 消息
    ...
    fun setHandsUpIcon(isShow: Boolean) {
        LogX.e("setHandsUpIcon isShow = $isShow")
        binding.handIcon.visibility = if (isShow) View.VISIBLE else View.GONE
    }
    ...
    override fun onCohostUpdated(item: AgoraUIUserDetailInfo, isCoHost: Boolean) {
        ...
        if (isCoHost) {
            ...
            if (item.role == Teacher) {
                ...
                eduContext?.userContext()?.getCoHostList()?.forEach {   // 找到 properties 中对应的用户
                    eduContext?.streamContext()?.getStreamList(it.userUuid)
                        ?.forEach { eduStreamInfo -> // 让该用户中的所有流大窗关闭
    ...
    override fun onPrivateChatUpdated(item: AgoraUIUserDetailInfo) {
        LogX.d(TAG, "onPrivateChatUpdated: ${GsonUtil.toJson(item)}")
        val event = AgoraTransportEvent(AgoraTransportEventId.EVENT_ID_OPTIONS_PANEL)
        event.extra = GsonUtil.gson.toJson(item)
