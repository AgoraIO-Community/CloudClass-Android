...
import io.agora.online.helper.AgoraUIConfig
...
class AgoraEduListVideoComponent : AbsAgoraEduComponent {
    ...
    private val layout = LayoutInflater.from(context).inflate(R.layout.fcr_online_edu_userlist_video_component, this, true)
    ...
    var handsUpList = ArrayList<String>()
    ...
    init {
        ...
        updateHandsUpList(FcrHandsUpManager.getList())

        FcrHandsUpManager.onHandsUpUpdateListener = {
            // update
            updateHandsUpList(it)

            val listData: MutableList<VideoItem> = differ.currentList
            val listDataV2 = ArrayList<VideoItem>()

            listData.forEachIndexed { index, item ->
                val videoItem = VideoItem(item.info, item.audioVolume, item.isHandsUp)
                videoItem.isHandsUp = handsUpList.contains(item.info.userUuid)
                listDataV2.add(videoItem)
            }

            LogX.e("data =${ differ.currentList } || data2 = ${listDataV2}")

            differ.submitList(listDataV2)
        }
    }

    fun updateHandsUpList(list: List<String>) {
        handsUpList.clear()
        ...
        differ.currentList.forEachIndexed { index, item ->
            if (item.info.streamUuid == streamUuid) {
                recyclerView.layoutManager?.findViewByPosition(index)?.let {
                    val rvLeft = this.left // this.left 当前 AgoraEduListVideoComponent 对象的 left
...
//            rightArrow.isVisible = getRvRightDistance(rv) > videoW
    ...
    private fun updateCoHostList(list: MutableList<AgoraUIUserDetailInfo>) {
        ...
        if (list.size > 0) {
            differ.submitList(list.map {
                //LogX.e("updateCoHostList","is = ${handsUpList.contains(it.userUuid)}")
                VideoItem(it, 0, handsUpList.contains(it.userUuid))
            } as MutableList<VideoItem>)
        } else {
    ...
    private inner class CoHostVideoAdapter(val callback: IAgoraUIVideoListener?) : RecyclerView.Adapter<VideoHolder>() {
        ...
        private val TAG = "CoHostVideoAdapter"
        ...
        override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): VideoHolder {
            if (classRoomType == RoomType.LARGE_CLASS) {
                return VideoHolder(
                    layoutInflater.inflate(R.layout.fcr_online_edu_userlist_video_item_v2, parent, false),
                    callback
                )
            }

            return VideoHolder(
                layoutInflater.inflate(R.layout.fcr_online_edu_userlist_video_item, parent, false),
                callback
            )
        }
        ...
        override fun onBindViewHolder(holder: VideoHolder, position: Int, payloads: MutableList<Any>) {
            ...
            LogX.e(TAG," payloads = $payloads || position = $position")
        }
...
internal class VideoListItemMatcher : DiffUtil.ItemCallback<VideoItem>() {
    ...
    override fun areContentsTheSame(oldItem: VideoItem, newItem: VideoItem): Boolean {
        return (oldItem.info.userName == newItem.info.userName
            ...
            && oldItem.audioVolume == newItem.audioVolume && oldItem.isHandsUp == newItem.isHandsUp)
    }
...
internal class VideoHolder(var view: View, var callback: IAgoraUIVideoListener?) : RecyclerView.ViewHolder(view),
    ...
    fun bind(roomUuid: String?, item: VideoItem, agoraUIProvider: IAgoraUIProvider, mCurView: AbsAgoraEduComponent) {
        ...
        uiVideo.setHandsUpIcon(item.isHandsUp)
    }
