...
import io.agora.online.easeim.EaseIM
...
import io.agora.online.easeim.utils.SoftInputUtil
import io.agora.online.easeim.view.`interface`.ChatPagerListener
import io.agora.online.easeim.view.`interface`.InputMsgListener
import io.agora.online.easeim.view.ui.widget.ChatViewPager
import io.agora.online.easeim.view.ui.widget.InputView
import io.agora.online.easeim.view.ui.widget.ShowImageView
...
import io.agora.online.impl.chat.AgoraChatInteractionSignal.UnreadTips
...
class AgoraEduEaseChatWidget : ChatPopupWidget(), InputMsgListener, ChatPagerListener {
    ...
    override fun init(container: ViewGroup) {
        ...
        LayoutInflater.from(mContext).inflate(R.layout.fcr_online_ease_chat_layout, null, false)?.let {
            layout = it
    ...
    private fun addEaseIM() {
        ...
        if (parseEaseConfigProperties()) {
            ...
            chatViewPager?.let {
                ...
                it.setRole(eduCore?.eduContextPool()?.userContext()?.getLocalUserInfo()?.role?.value?: EaseConstant.ROLE_STUDENT)
...
//            chatViewPager?.isNeedRoomMutedStatus = eduC
            ...
            mContext?.let {
                ...
                userSearchView = UserSearchView(it)
                userSearchView?.let { list ->
                    val params = RelativeLayout.LayoutParams(
                        ViewGroup.LayoutParams.MATCH_PARENT,
                        ViewGroup.LayoutParams.MATCH_PARENT
                    )
...
//        val properties = eduContext?.widgetContext2()?.getWidgetProperties(WidgetType.IM)
    ...
    override fun onMsgReceiverClick() {
        userSearchView?.initView(eduCore!!, mCurrentPrivateReceiver, MySearchResultCallback())
        userSearchView?.visibility = View.VISIBLE
    }
    ...
    override fun setPrivateChatViewDisplayed(user: AgoraUIUserDetailInfo?, displayed: Boolean) {
        chatViewPager?.setPrivateChatViewDisplayed(user, displayed)
    }
    ...
    inner class MySearchResultCallback : UserSearchView.SearchResultCallback {
        override fun onUserSelected(user: AgoraEduContextUserInfo?) {
            if (user == null) {
                chatViewPager?.setPrivateChatViewDisplayed(null, false)
            }else {
                val currentPrivateReceiver = AgoraUIUserDetailInfo(
                    AppUtil.toMD5String(user.userUuid),
                    user.userName,
...
//            if (ChatClient.getInstance().currentUser.equals(currentPrivateReceiver.userUuid)) {
//                return
//            }

                chatViewPager?.setPrivateChatViewDisplayed(currentPrivateReceiver, true)
            }
            userSearchView?.visibility = View.INVISIBLE
        }
