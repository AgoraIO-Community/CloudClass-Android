...
import io.agora.online.component.teachaids.CountdownLaunchStatus.Init
import io.agora.online.component.teachaids.CountdownLaunchStatus.Started
import io.agora.online.component.teachaids.CountdownLaunchStatus.Paused
import io.agora.online.component.teachaids.CountdownStatics.DEFAULT_DURATION
import io.agora.online.component.teachaids.CountdownStatics.MAX_DURATION
import io.agora.online.component.teachaids.CountdownStatics.PROPERTIES_KEY_DURATION
import io.agora.online.component.teachaids.CountdownStatics.PROPERTIES_KEY_START_TIME
import io.agora.online.component.teachaids.CountdownStatics.PROPERTIES_KEY_STATE
...
import io.agora.online.databinding.FcrOnlineCountdownWidgetContentBinding
import io.agora.online.databinding.FcrOnlineViewSimpleClockBinding
...
class AgoraTeachAidCountDownWidget : AgoraTeachAidMovableWidget() {
    ...
    private var countDownContent: AgoraTeachAidCountDownWidget.AgoraCountDownWidgetContent? = null
    ...
    override fun init(container: ViewGroup) {
        ...
        container.post {
            widgetInfo?.localUserInfo?.let {
                ...
                LogX.e(TAG, "init AgoraCountDownWidget success")
            }
        }
    }
    ...
    private inner class AgoraCountDownWidgetContent(val container: ViewGroup, val localUserInfo: AgoraWidgetUserInfo) {
        ...
        private val binding = FcrOnlineCountdownWidgetContentBinding.inflate(
        ...
        fun parseProperties(properties: Map<String, Any>) {
            ...
            when (state) {
                ...
                Paused.value -> {
                    // first enter room, countdown is not started, but teacher has paused countdown
                    setSpecificCountdownTime(duration * 1000L)
                    binding.countdownClock.pauseCountdown()
                }
...
//            }
            ...
            if (binding.countdownClock.isAttachedToWindow) {
                binding.countdownClock.setCountdownListener(object :
...
//                }
        ...
        private fun switchSelf(close: Boolean = true, extraProperties: Map<String, Any>? = null) {
            val packet = AgoraTeachAidWidgetInteractionPacket(
                ActiveState,
                AgoraTeachAidWidgetActiveStateChangeData(
                    !close,
                    extraProperties
                )
            )
...
class AlignedTextView : androidx.appcompat.widget.AppCompatTextView {
    private var alignment =
        AlignedTextView.ProperTextAlignment.TOP
    ...
    override fun onDraw(canvas: Canvas?) {
        canvas?.let {
            ...
            var drawY = 0f
            if (alignment == AlignedTextView.ProperTextAlignment.TOP) {
                drawY = (textRect.bottom - bottom).toFloat() - ((textRect.bottom - textRect.top) / 2)
            } else if (alignment == AlignedTextView.ProperTextAlignment.BOTTOM) {
    ...
    private fun setAlignment(alignment: Int) {
        if (alignment == 1) {
            ...
            this.alignment =
                AlignedTextView.ProperTextAlignment.BOTTOM
        }
    }
...
class CountDownClock : LinearLayout {
    ...
    private var countdownListener: CountDownClock.CountdownCallBack? = null
    ...
    private val binding = FcrOnlineViewSimpleClockBinding.inflate(
    ...
    constructor(context: Context?, attrs: AttributeSet?) : this(context, attrs, 0)
    constructor(context: Context?, attrs: AttributeSet?, defStyleAttr: Int) : super(context, attrs, defStyleAttr) {
        View.inflate(context, R.layout.fcr_online_view_simple_clock, this)
    ...
    fun setCountdownListener(countdownListener: CountDownClock.CountdownCallBack) {
...
class CountDownDigit : FrameLayout {
    ...
    private val binding = FcrOnlineCountdownClockDigitBinding.inflate(
        LayoutInflater.from(context),
        this, true
    )
...
internal object CountdownStatics {
    ...
    const val PROPERTIES_KEY_DURATION = "duration" // 单位：秒
    const val PROPERTIES_KEY_TOTAL_DURATION = "totalDuration"
