        ...
        RetrofitManager.instance()!!.getService(AgoraEduSDK.baseUrl(), UserService::class.java)
                ...
                .enqueue(RetrofitManager.Callback(0, object : ThrowableCallback<ResponseBody<EduEntryRes>> {
                    override fun onSuccess(res: ResponseBody<EduEntryRes>?) {
                        ...
                        /**
                         * enableEncryption if needed
                         */
//                        if(mediaOptions.encryptionConfigs !== null) {
//                            AgoraLog.i("$TAG->enable encryption")
//                            RteEngineImpl.enableEncryption(true, mediaOptions.rteEncryptionConfig())
//                        }

                        /**加入rte(包括rtm和rtc)*/
                        joinRte(rtcToken, roomEntryRes.user.streamUuid.toLong(),
                                mediaOptions.convert(), options.tag, mediaOptions.rteEncryptionConfig(), object : RteCallback<Void> {
    ...
    private fun joinRte(rtcToken: String, rtcUid: Long, channelMediaOptions: ChannelMediaOptions,
                        tag: Int?, encryptionConfig: EncryptionConfig, @NonNull callback: RteCallback<Void>) {
        ...
        RteEngineImpl[getCurRoomUuid()]?.join(rtcOptionalInfo, rtcToken, rtcUid, channelMediaOptions, encryptionConfig, callback)
    }
