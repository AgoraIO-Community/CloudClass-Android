...
internal class EduManagerImpl(
    ...
    init {
        /*初始化LogManager*/
        if (options.logFileDir.isNullOrEmpty()) {
            val path = options.context?.cacheDir.toString().plus(File.separatorChar).plus(LOGS_DIR_NAME)
            options.logFileDir = path
        }
        options.context?.let {
            /*注册UnCatchExceptionHandler*/
            UnCatchExceptionHandler.getExceptionHandler().init(it.applicationContext,
...
//        }
    ...
    override fun createClassroom(config: RoomCreateOptions): EduRoom? {
        ...
        val status = EduRoomStatus(EduRoomState.INIT, 0, true,
                0, 0)
