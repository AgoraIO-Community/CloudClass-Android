...
public class RoomPreImpl extends Base implements RoomPre {
    ...
    @Override
    public void updateCameraState(String userUuid, boolean cameraDeviceAvailable) {
        RetrofitManager.instance().getService(AgoraEduSDK.baseUrl(), RoomPreService.class)
                .updateCameraState(appId, roomUuid, userUuid, new CameraStateUpdateReq(
                        cameraDeviceAvailable ? 1 : 0))
