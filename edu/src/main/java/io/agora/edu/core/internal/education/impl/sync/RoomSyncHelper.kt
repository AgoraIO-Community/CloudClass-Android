package io.agora.edu.core.internal.education.impl.sync
...
import io.agora.edu.core.internal.education.impl.cmd.CMDDataMergeProcessor
...
import io.agora.edu.core.internal.server.struct.response.EduSequenceListRes
import io.agora.edu.core.internal.server.struct.response.EduSequenceRes
import io.agora.edu.core.internal.server.struct.response.EduSequenceSnapshotRes
...
internal class RoomSyncHelper(private val eduRoom: EduRoom, roomInfo: EduRoomInfo,
    ...
    /**一个同步流程中的id集合*/
    private var sequenceList: MutableList<Int> = CopyOnWriteArrayList()

    /**一个同步流程中的cmd数据集合*/
    private var sequenceData: MutableMap<Int, EduSequenceRes<Any>> = ConcurrentHashMap<Int, EduSequenceRes<Any>>(16)

    private fun clearSequence() {
        sequenceList.clear()
        sequenceData = ConcurrentHashMap<Int, EduSequenceRes<Any>>()
    }
    ...
    override fun updateSequenceId(cmdResponseBody: CMDResponseBody<Any>): Pair<Int, Int>? {
        ...
        if (syncing || (eduRoom as EduRoomImpl).joining) {
            AgoraLog.w("$TAG->The messages(${cmdResponseBody.sequence}) received during the join process or the synchronous " +
                    "sequence process are added to the cache->syncing:$syncing, joining:${(eduRoom as EduRoomImpl).joining}")
...
//        clearSequence()
        ...
        cache.list.forEach {
            if (!sequenceList.contains(it.sequence)) {
                ...
                sequenceData[it.sequence] = Convert.convertCMDResponseBody(it)
            }
        }
        sequenceList.sort()
        sequenceList.forEach {
            val cmdRes = Convert.convertEduSequenceRes(sequenceData[it] as EduSequenceRes<Any>)
            val pair = updateSequenceId(cmdRes)
            if (pair != null) {
                fetchLostSequence(pair.first, pair.second, callback)
                return
            }
    ...
    override fun fetchLostSequence(nextId: Int, count: Int?, callback: EduCallback<Unit>) {
        ...
        RetrofitManager.instance()!!.getService(AgoraEduSDK.baseUrl(), RoomService::class.java)
                ...
                .enqueue(RetrofitManager.Callback(0, object : ThrowableCallback<DataResponseBody<EduSequenceListRes<Any>>> {
                    override fun onSuccess(res: DataResponseBody<EduSequenceListRes<Any>>?) {
                        AgoraLog.i("$TAG->The missing sequence data requested from $nextId is :${Gson().toJson(res)}")
                        val tmp = res?.data?.list
                        if (tmp != null && tmp.size > 0) {
                            /**把缺失的seq数据添加到集合中*/
                            addSequenceData(res.data)
                            /**join成功后，自行处理缓存中的数据*/
    ...
    override fun fetchSnapshot(callback: EduCallback<Unit>) {
        ...
        RetrofitManager.instance()!!.getService(AgoraEduSDK.baseUrl(), RoomService::class.java)
                ...
                .enqueue(RetrofitManager.Callback(0, object : ThrowableCallback<DataResponseBody<EduSequenceSnapshotRes>> {
                    override fun onSuccess(res: DataResponseBody<EduSequenceSnapshotRes>?) {
