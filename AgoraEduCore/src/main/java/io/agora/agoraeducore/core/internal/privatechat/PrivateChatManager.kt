package io.agora.agoraeducore.core.internal.privatechat
...
import io.agora.agoraeducore.core.internal.edu.classroom.BaseManager
import io.agora.agoraeducore.core.internal.edu.classroom.bean.PropertyData
import io.agora.agoraeducore.core.internal.edu.common.bean.sidechat.SideChatConfig
import io.agora.agoraeducore.core.internal.edu.common.bean.sidechat.SideChatStream
...
import io.agora.agoraeducore.core.internal.education.api.stream.data.VideoStreamType
...
internal class PrivateChatManager(
        context: Context,
        eduContextPool: EduContextPool,
        currentRoom: EduRoom?,
        config: AgoraEduCoreConfig,
        eduUser: EduLocalUser) : BaseManager(context, config, currentRoom, eduUser, eduContextPool) {
    override var tag = "PrivateChatManager"

    private val privateChatContext = eduContextPool.privateChatContext()
    ...
    fun startPrivateChat(toUserUuid: String, callback: Callback<ResponseBody>): Boolean {
        ...
        RetrofitManager.instance().getService(AgoraEduSDK.baseUrl(), PrivateChatService::class.java)
                .startPrivateChat(config.appId, config.roomUuid, toUserUuid)
                .enqueue(RetrofitManager.Callback<ResponseBody>(0, object : ThrowableCallback<ResponseBody?> {
                    override fun onSuccess(res: ResponseBody?) {
                        ...
                        setPrivateChat(true, config.userUuid, toUserUuid)
                    }
    ...
    fun stopSideVoiceChat(callback: Callback<ResponseBody>): Boolean {
        ...
        RetrofitManager.instance().getService(AgoraEduSDK.baseUrl(), PrivateChatService::class.java)
                .finishPrivateChat(config.appId, config.roomUuid, toUserUuid!!)
                .enqueue(RetrofitManager.Callback<ResponseBody>(0, object : ThrowableCallback<ResponseBody?> {
                    override fun onSuccess(res: ResponseBody?) {
                        callback.onSuccess(res)
            ...
            override fun onSuccess(res: EduLocalUser?) {
                res?.let { localUser ->
                    ...
                    config?.users?.forEach { user ->
                        if (user.userUuid == this@PrivateChatManager.config.userUuid) {
    ...
    private fun handleGroupStreamRules(localUser: EduLocalUser, streamId: String, subscribe: Boolean,
    ...
    private fun handleDefaultStreamRules(localUser: EduLocalUser, userId: String) {
    ...
    private fun handleGroupStreamRules(localUser: EduLocalUser, streamInfo: EduStreamInfo,
                                       subscribe: Boolean, hasAudio: Boolean, hasVideo: Boolean) {
        eduRoom?.let {
            if (subscribe && streamInfo.publisher.userUuid != config.userUuid) {
